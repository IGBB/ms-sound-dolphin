...
other attached packages:
 [1] cowplot_1.1.1       treemapify_2.5.6    haplotypes_1.1.3.1
 [4] poppr_2.9.6         hierfstat_0.5-11    aplot_0.2.1
 [7] ascii_2.4           hues_0.2.0          ggrepel_0.9.3
[10] ggsankey_0.0.99999  maps_3.4.2          Biostrings_2.66.0
[13] GenomeInfoDb_1.34.9 XVector_0.38.0      IRanges_2.32.0
[16] S4Vectors_0.36.2    BiocGenerics_0.44.0 ggtree_3.6.2
[19] tidytree_0.4.5      lubridate_1.9.2     forcats_1.0.0
[22] stringr_1.5.1       dplyr_1.1.4         purrr_1.0.2
[25] readr_2.1.4         tidyr_1.3.1         tibble_3.2.1
[28] ggplot2_3.5.1.9000  tidyverse_2.0.0     LEA_3.10.2
[31] pegas_1.3           ape_5.7-1           adegenet_2.1.10
[34] ade4_1.7-22

loaded via a namespace (and not attached):
 [1] colorspace_2.1-0       seqinr_4.2-36
 [3] ellipsis_0.3.2         fs_1.6.3
 [5] farver_2.1.2           ggfittext_0.10.2
 [7] fansi_1.0.6            codetools_0.2-19
 [9] splines_4.2.2          cachem_1.0.8
...
[37] phangorn_2.11.1        zlibbioc_1.44.0
[39] MASS_7.3-60            scales_1.3.0
[41] hms_1.1.3              promises_1.2.1
[43] parallel_4.2.2         RColorBrewer_1.1-3
[45] memoise_2.0.1          ggfun_0.1.3
[47] yulab.utils_0.1.0      stringi_1.8.4
[49] plotrix_3.8-4          permute_0.9-7
[51] boot_1.3-30            rlang_1.1.3
[53] pkgconfig_2.0.3        bitops_1.0-7
[55] polysat_1.7-7          lattice_0.21-8
[57] treeio_1.22.0          patchwork_1.2.0.9000
...
[75] data.table_1.14.8      vegan_2.6-6.1
[77] digest_0.6.33          xtable_1.8-4
[79] httpuv_1.6.12          gridGraphics_0.5-1
[81] munsell_0.5.1          viridisLite_0.4.2
[83] ggplotify_0.1.2        tcltk_4.2.2
[85] quadprog_1.5-8
#+end_example
...
** Load alignments
#+begin_src R
fasta.full <- read.dna("1-mafft-with-outgroup.fasta",
...
haplotypes.full.dist.order <-
  hclust(haplotypes.fill.dist)[c("labels", "order")] %>%
  bind_cols %>%
  mutate(new = labels[order]) %>%
  pull(new) %>%
  rev

haplotypes.fill.dist %>%
  as.matrix %>%
  as.data.frame %>%
...
plots.structure.hap.all <- lapply(2:9, function(x) {
  ...
  member.data.order <- spread(member.data, pop, q) %>%
    ...
    as.dendrogram %>%
    order.dendrogram
  haplotypes.names <- snps.whole@ind.names
...
legend <-
  get_plot_component(plot.distance.hap +
                     geom_point(aes(color=pop),na.rm = T) +
                     scale_color_iwanthue() +
                     guides(color=guide_legend(
                              title="Population",
                              override.aes = list(size=5),
                              theme = theme(legend.text.position =
                                              "top")),
                            fill=guide_colorbar(title="Distance"))+
...
sequence.membership.hap %>%
  ...
  summarise(Haplotypes=n(),
    ...
    - pop4 q values
      #+begin_src R :results value table :colnames yes
filter(member.data.hap, pop == 4 & q > 0.0001) %>%
  ...
  rename("ID"="mtCR-hap", "best.pop"='mtCR-pop')
     ...
     #+RESULTS:
     | P | Haplo    |      q | Sample      | mtCR-hap    | mtCR-pop   |
     |---+----------+--------+-------------+-------------+------------|
     | 1 | mito-062 | 0.2250 | SER11-0141  | mtCR.mix-11 | mtCR.inner |
     | 4 | mito-097 | 0.9997 | SER19-00888 | mtCR.mix-10 | mtCR.outer |
...
sequence.membership.hap %>%
  ...
  as.data.frame
      ...
      |------------+------------+----------+---------+----------+---------+-------+------+-----|
      | mtCR.mix-3 |          2 |       63 |       6 |        3 |       1 |    75 |    1 |  46 |
      | mtCR.mix-3 |          3 |       63 |       6 |        3 |       1 |    75 |    1 |  46 |
      | mtCR.mix-5 |          2 |        2 |         |          |         |     5 |      |  22 |
      | mtCR.mix-5 |          3 |        2 |         |          |         |     5 |      |  22 |
      | mtCR.mix-7 |          2 |          |         |          |         |    11 |      |   8 |
      | mtCR.new-5 |          2 |          |         |          |         |       |      |   1 |
...
#+HEADER: :results output graphics file
#+HEADER: :file full-mito-nj-tree-circular.png
#+header: :width 12 :height 12 :units in :res 200 :bg white
#+begin_src R
sequence.membership <- read.delim("4-struct-membership.K4.tab",
                                  col.names = c("ID", "clade")) %>%
  mutate(clade=paste("population", clade))

anno <- data.frame(ID=tree.data$tip.label) %>%
  left_join(sequence.membership)

anno[anno$ID=="SER19-00888","label"] <- "SER19-00888"
anno[anno$ID=="SER11-0141","label"]  <- "SER11-0141"
anno[1,"label"] <- "NC_060612.1"

layout <- ggtree(tree.data, layout = "circular") %<+% sequence.membership
...
p
      ...
      #+RESULTS:
      [[file:full-mito-nj-tree-circular.png]]
...
lapply(fasta.split, nuc.div, variance = T) %>%
  ...
  | Population |       pi |        var |
  |------------+----------+------------|
  |          1 | 0.000180 | 1.1499e-08 |
  |          2 |          |            |
  |          3 | 0.000309 | 2.8008e-08 |
  |          4 | 0.000172 | 1.0096e-08 |
  |        all | 0.000507 | 6.7727e-08 |
  :end:
...
genind.all <- DNAbin2genind(fasta.full[-1,], pop=pop.all)
strata(genind.all) <- data.frame(pop.all)

set.seed(168495)
pop <- pop.all
pop[pop == '1'] <- paste0( pop[pop == '1'],
          letters[sample.int(6, size = sum(pop==1), replace = T)])
pop[pop == '3'] <- paste0( pop[pop == '3'],
          letters[sample.int(3, size = sum(pop==3), replace = T)])
pop <- factor(pop)

genind.subdivide <- DNAbin2genind(fasta.full[-1,], pop=pop)
...
pair <-
  ...
  filter(PopA < PopB) %>%
  mutate(PopA = substr(PopA, 0, 1),
         PopB = substr(PopB, 0, 1)) %>%
  group_by(PopA, PopB) %>%
...
rbind(data.frame(PopA='all', PopB='all', Fst=all$FST, class=NA), pair)
  ...
  | PopA | PopB |                   Fst | class      |
  |------+------+-----------------------+------------|
  |  all |  all |    0.0492258753638389 |            |
  |    1 |    3 |  -0.00631719883551522 |            |
  |    1 |    2 |    0.0234100268611076 | little     |
  |    2 |    3 |    0.0244663797874993 | little     |
  |    3 |    4 |      0.43866294973329 | very great |
  |    1 |    4 |     0.471230894435597 | very great |
  |    2 |    4 |     0.644444444444445 | very great |
4) PhiST
   - Global
     #+begin_src R
...
phistp
    ...
    $results
                     Df     Sum Sq   Mean Sq
    Between samples   3   47.51126 15.837088
    Within samples  382 2921.41483  7.647683
    Total           385 2968.92609  7.711496

    $componentsofcovariance
                                   Sigma          %
    Variations  Between samples 0.120298   1.548639
    Variations  Within samples  7.647683  98.451361
    Total variations            7.767981 100.000000

    $statphi
                             Phi
    Phi-samples-total 0.01548639
    #+end_example
...
phist.table <- expand_grid(pop1=levels(pop),
  ...
  ungroup
    ...
    Contracted multilocus genotypes ... 36
    ...
    [1] "1d"
    Filtering ...
    Original multilocus genotypes   ... 41
    Contracted multilocus genotypes ... 37

    Found 307 missing values.
    ...
    [1] "1e"
    Filtering ...
    Original multilocus genotypes   ... 39
    Contracted multilocus genotypes ... 35
    ...
    [1] "3c"
    Filtering ...
    Original multilocus genotypes   ... 36
    ...
    [1] "1e"
    Filtering ...
    Original multilocus genotypes   ... 38
    Contracted multilocus genotypes ... 35

    Found 230 missing values.
    ...
    [1] "1f"
    Filtering ...
    Original multilocus genotypes   ... 34
    Contracted multilocus genotypes ... 32
    ...
    [1] "2"
    Filtering ...
    Original multilocus genotypes   ... 24
    Contracted multilocus genotypes ... 24

    Found 143 missing values.
    ...
    [1] "4"
    Filtering ...
    Original multilocus genotypes   ... 20
    ...
    Filtering ...
    Original multilocus genotypes   ... 41
    Contracted multilocus genotypes ... 38

    Found 468 missing values.
    ...
    [1] "1e"
    Filtering ...
    Original multilocus genotypes   ... 43
    Contracted multilocus genotypes ... 38

    Found 399 missing values.
    ...
    [1] "1e"
    Filtering ...
    Original multilocus genotypes   ... 42
    Contracted multilocus genotypes ... 38

    Found 385 missing values.
    ...
    #+begin_src R  :results value table :colnames yes
phist.table %>%
  separate(pop1, into=c("pop1", "pop1.sub"), sep=c(1)) %>%
  separate(pop2, into=c("pop2", "pop2.sub"), sep=c(1)) %>%
  filter(pop1 != pop2) %>%
  group_by(pop1, pop2) %>%
  ...
  arrange(phist)
    ...
    | pop1 | pop2 |                phist |
    |------+------+----------------------|
    |    1 |    3 | -0.00374438566705228 |
    |    1 |    2 |   0.0238006447152306 |
    |    2 |    3 |   0.0244725148636873 |
    |    3 |    4 |     0.43866832939318 |
    |    1 |    4 |    0.472956674811125 |
    |    2 |    4 |    0.644444444444445 |
5) Private alleles
    #+begin_src R :results value table  :colnames yes
 ...
 private.alleles
    ...
    |---------+---------+---------+------+---------------------|
    |       1 | 3308.c  |       1 |  237 | 0.00421940928270042 |
    |       1 | 7761.a  |       1 |  237 | 0.00421940928270042 |
    |       1 | 12501.t |       1 |  237 | 0.00421940928270042 |
    |       1 | 12980.t |       1 |  237 | 0.00421940928270042 |
    |       1 | 12980.c |       1 |  237 | 0.00421940928270042 |
